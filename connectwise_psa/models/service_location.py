...
class ServiceLocation(BaseModel):
    ...
    where: Optional[StrictStr]
    __properties: ClassVar[List[str]] = ["_info", "defaultFlag", "id", "name", "where"]

    @field_validator('where')
    def where_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('OnSite', 'Remote', 'InHouse'):
            raise ValueError("must be one of enum values ('OnSite', 'Remote', 'InHouse')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in ServiceLocation) in the input: " + _key)
