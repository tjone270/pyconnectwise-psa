...
class MenuEntryLocation(BaseModel):
    ...
    menu_entry: Optional[SystemMenuEntryReference] = Field(default=None, alias="menuEntry")
    __properties: ClassVar[List[str]] = ["_info", "id", "location", "menuEntry"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of menu_entry
        if self.menu_entry:
            _dict['menuEntry'] = self.menu_entry.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in MenuEntryLocation) in the input: " + _key)

        _obj = cls.model_validate({
            "_info": obj.get("_info"),
            "id": obj.get("id"),
            "location": SystemLocationReference.from_dict(obj.get("location")) if obj.get("location") is not None else None,
            "menuEntry": SystemMenuEntryReference.from_dict(obj.get("menuEntry")) if obj.get("menuEntry") is not None else None
        })
        return _obj
