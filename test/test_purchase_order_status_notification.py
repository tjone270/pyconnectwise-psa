# coding: utf-8

"""
    ConnectWise PSA

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 2022.2
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

from connectwise_psa.models.purchase_order_status_notification import PurchaseOrderStatusNotification

class TestPurchaseOrderStatusNotification(unittest.TestCase):
    """PurchaseOrderStatusNotification unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PurchaseOrderStatusNotification:
        """Test PurchaseOrderStatusNotification
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PurchaseOrderStatusNotification`
        """
        model = PurchaseOrderStatusNotification()
        if include_optional:
            return PurchaseOrderStatusNotification(
                info = {
                    'key' : ''
                    },
                email = '',
                id = 56,
                member = connectwise_psa.models.member_reference.MemberReference(
                    _info = {
                        'key' : ''
                        }, 
                    id = 56, 
                    identifier = '', 
                    name = '', ),
                notify_who = connectwise_psa.models.notification_recipient_reference.NotificationRecipientReference(
                    _info = {
                        'key' : ''
                        }, 
                    id = 56, 
                    identifier = '', 
                    name = '', ),
                status = connectwise_psa.models.purchase_order_status_reference.PurchaseOrderStatusReference(
                    _info = {
                        'key' : ''
                        }, 
                    id = 56, 
                    name = '', ),
                workflow_step = 56
            )
        else:
            return PurchaseOrderStatusNotification(
        )
        """

    def testPurchaseOrderStatusNotification(self):
        """Test PurchaseOrderStatusNotification"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
